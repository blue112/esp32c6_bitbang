...
#define CHIP_ID_ESP32_C6 0x2CE0806F
...
{
  int fd = *(int *)arg;
  // printf("Writting 0x%02X to UART\n", ch);
  if (write(fd, &ch, 1) != 1)
    fail("failed to write %d to fd %d", ch, fd);
}
...
static int iowait(int fd, int sock, int ms)
{
  ...
  if (sock > 0)
    FD_SET(sock, &rset);
  if (select((fd > sock ? fd : sock) + 1, &rset, 0, 0, &tv) < 0)
    FD_ZERO(&rset);
  if (FD_ISSET(0, &rset))
    ready |= 1;
  if (FD_ISSET(fd, &rset))
    ready |= 2;
  if (sock > 0 && FD_ISSET(sock, &rset))
    ready |= 4;
  return ready;
}
               ...
               uint32_t cs, int timeout_ms)
{
  ...
  if (ctx->verbose)
    dump(cmdstr(op), tmp, 8 + len); // Hexdump if required
  ...
  for (;;)
  {
    ...
    if (!(ready & 2))
      return 1;                          // Interrupted, fail
    n = read(ctx->fd, tmp, sizeof(tmp)); // Read from a device
    if (n <= 0)
      fail("Serial line closed\n"); // Doh. Unplugged maybe?
    // if (ctx->verbose) dump("--RAW_RESPONSE:", tmp, n);
    for (i = 0; i < n; i++)
    {
      size_t r = slip_recv(tmp[i], &ctx->slip); // Pass to SLIP state machine
      if (r == 0 && ctx->slip.mode == 0)
        putchar(tmp[i]); // In serial mode
      if (r == 0)
        continue;
      if (ctx->verbose)
        dump("--SLIP_RESPONSE:", ctx->slip.buf, r);
      if (r < 10 || ctx->slip.buf[0] != 1 || ctx->slip.buf[1] != op)
        continue;
...
{
  size_t i, nchips;
  if (read32(ctx, 0x40001000, &ctx->chip.id))
    fail("Error reading chip ID\n");
  nchips = sizeof(s_known_chips) / sizeof(s_known_chips[0]);
  for (i = 0; i < nchips; i++)
  {
    if (s_known_chips[i].id == ctx->chip.id)
...
static bool chip_connect(struct ctx *ctx)
{
  ...
  for (i = 0; i < 50; i++)
  {
    ...
    if (cmd(ctx, 0x08, data, sizeof(data), 0, 250) == 0)
    {
      sleep_ms(50);
      flushio(ctx->fd); // Discard all data
      chip_detect(ctx);
      if (ctx->verbose)
        printf("Connected to chip!\n");
      return true;
    }
    flushio(ctx->fd);
  }
...
static void monitor(struct ctx *ctx)
{
  ...
  if (ready & 2)
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      if (len == 0 && ctx->slip.mode == 0)
        putchar(buf[i]); // In serial mode
      if (len <= 0)
        continue;
      ...
      if (ctx->verbose)
        dump("SR", ctx->slip.buf, len);
    }
    fflush(stdout);
  }
  if (ready & 1)
  { // Forward stdin to a device
    uint8_t buf[BUFSIZ];
    int n = read(0, buf, sizeof(buf));
    if (n > 0 && ctx->verbose)
      dump("WRITE", buf, n);
    for (i = 0; i < n; i++)
      uart_tx(buf[i], &ctx->fd);
  }
  if (ready & 4)
  { // Something in the UDP socket
    uint8_t buf[2048];
...
static void spiattach(struct ctx *ctx)
{
  ...
  if (cmd(ctx, 13, d3, sizeof(d3), 0, 250))
    fail("SPI_ATTACH failed\n");
  // flash_id, flash size, block_size, sector_size, page_size, status_mask
  if (cmd(ctx, 11, d4, sizeof(d4), 0, 250))
    fail("SPI_SET_PARAMS failed\n");
}
...
static int unhex(const char *hexfile, const char *dir, char *buf, size_t bl)
{
  ...
  if (in == NULL)
    return fail("ERROR: cannot open %s\n", hexfile);
  mkdir(dir, 0755);
  buf[0] = '\0';
                     ...
                     uint32_t flash_offset, const char *path)
{
  ...
  while ((n = fread(buf + hs, 1, block_size, fp)) > 0)
  {
    ...
    for (i = 0; i < 100; i++)
      putchar('\b');
    printf("Writing %s, %d/%d bytes @ 0x%x (%d%%)\n", path, n, size,
...
static void flash(struct ctx *ctx, const char **args)
{
  ...
  if (!chip_connect(ctx))
    fail("Error connecting\n");
  if (ctx->fpar != NULL)
    flash_params = (uint16_t)strtoul(ctx->fpar, NULL, 0);
  if (atoi(ctx->baud) > 115200)
  {
    uint32_t data[] = {atoi(ctx->baud), 0};
    if (cmd(ctx, 15, data, sizeof(data), 0, 50))
      fail("SET_BAUD failed\n");
...

  {
    ...
    if (cmd(ctx, 4, d3, sizeof(d3), 0, 250))
...
static struct Elf32_Phdr elf_get_phdr(const struct mem *elf, int no)
{
  ...
  if (h->p_filesz == 0)
    no++; // GCC-generated phdrs have empty 1st phdr
  return h[no];
}
...
static int mkbin(const char *elf_path, const char *bin_path, bool verbose)
{
  ...
  if (elf.len < (int)sizeof(struct Elf32_Phdr))
    fail("corrupt ELF file\n");
  if (elf.ptr[4] != '\x01')
    fail("Not ELF32: %d\n", elf.ptr[4]);
  ...
  common_hdr[2] = 0x02;                                  // flash mode DIO
  common_hdr[3] = 0x10;                                  // flash size = 2MB, flash freq = 80MHz
  extended_hdr[4] = 13;                                  // Chip id = c6
  extended_hdr[9] = 0x63;                                // Max chip revision
  ...
  for (i = 0; i < num_segments; i++)
  {
    ...
    for (j = 0; j < aligned_size - h.p_filesz; j++)
      fputc(zero, bin_fp);
    ...
    for (;;)
    {
      ...
      if (n >= (int)(sizeof(buf) / sizeof(buf[0])) || c == EOF)
      {
        if (addr >= 0xffff && !(addr & 0xffff))
          printhexhiaddrline(addr);
        if (n > 0)
          printhexline(0, n, addr & 0xffff, buf);
        addr += n;
        n = 0;
      }
      if (c == EOF)
...
int main(int argc, const char **argv)
{
  ...
  if (ctx.port == NULL)
    ctx.port = "/dev/ttyACM0";

  if (ctx.baud == NULL)
    ctx.baud = "115200"; // Default baud rate
  if (temp_dir == NULL)
    temp_dir = "tmp"; // Default temp dir
  if (udp_port == NULL)
    udp_port = "1999"; // Default UDP_PORT
